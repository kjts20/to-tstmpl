...
const ignorePatternList = (function(){
    const patternList = (function(){
        try {
            return fs.readFileSync('.gitignore').toString().split('\n');
        } catch (e) {
    ...
    return patternList.map(it => new RegExp(it.replace(/\./, '\\.')));
})();
...
const fileName2VarName = function(fileName){
    return (fileName + '').split('.').map((it, i)=> i === 0?it:firstUpcase(it)).join('');
}
// 文件名字转命名
const toGlobalExportVarName = function(fileName){
    const paths = fileName.split('/');
    const lastIndex = paths.length - 1;
    return paths.map((it,i)=> {
        let name  = it
        if(i === lastIndex){
...
const project2TsTmpl = function(dirName, tsTmplName){
    ...
    const toAbsPath = (fullPath) => {
        const absPath = fullPath.replace(dirName, '').replace(/^(\\|\/)(.+)$/,'$2').replace('\\', '/');
    ...
    scanFileOrFolder(dirName, {
        fileHanlder: function(fileName){
            ...
            if(!isIgnore(fullname)){
                ...
                const fileContent = [
                    'const ' + fileVarName + 'File = `\n' + fs.readFileSync(fileName) + '\n`;',
                    `export const ${toGlobalExportVarName(fullname)} = {file: ${fileVarName}File, name: [${pathAndFile.map(it=> "'" + it + "'")}], ext:'${trimFirstPoint(ext)}'};` 
                ].join('\n'); 
